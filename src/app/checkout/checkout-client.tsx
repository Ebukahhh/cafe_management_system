...
function CreditCardIcon() { return <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" ry="2" /><line x1="1" y1="10" x2="23" y2="10" /></svg>; }
...
export default function CheckoutClient({ dynamicSlots, userProfile }: CheckoutClientProps) {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const res = await submitOrder(cartData, checkoutDetails);
      if (res.success) {
        clearCart();
        router.push(`/order-tracking/${res.orderId}`);
      }
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
      ...
      <div className="lg:w-[60%] space-y-10">
        ...
        {error && (
          <div className="bg-red-500/10 text-red-500 p-4 rounded-xl font-bold border border-red-500/20">
            {error}
          </div>
        )}
        ...
        <section className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3 md:gap-4">
            <button 
              ...
              className={`cursor-pointer p-4 rounded-xl transition-all ${orderType === 'pickup' ? 'bg-surface-container-high ring-2 ring-primary text-primary' : 'bg-surface-container-low hover:bg-surface-container-high text-on-surface/40'}`}
            >
            ...
            <button 
              ...
              className={`cursor-pointer p-4 rounded-xl transition-all ${orderType === 'delivery' ? 'bg-surface-container-high ring-2 ring-primary text-primary' : 'bg-surface-container-low hover:bg-surface-container-high text-on-surface/40'}`}
            >
              <div className="flex flex-col items-center text-center gap-2">
                <DeliveryIcon />
            ...
            <button 
               ...
               className={`cursor-pointer p-4 rounded-xl transition-all ${orderType === 'dine_in' ? 'bg-surface-container-high ring-2 ring-primary text-primary' : 'bg-surface-container-low hover:bg-surface-container-high text-on-surface/40'}`}
            >
              <div className="flex flex-col items-center text-center gap-2">
                <DineInIcon />
            ...
            <div className="flex-grow">
               <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                 <button 
                    ...
                    className={`px-4 py-2.5 rounded-lg font-bold text-sm cursor-pointer transition-all ${pickupTime === 'ASAP' ? 'bg-primary text-deep-espresso ring-2 ring-primary' : 'bg-surface-container-highest text-on-surface hover:bg-surface-bright'}`}
                 >
                   ASAP
                 </button>
                 {timeSlots.map((slot) => (
                   <button
                     ...
                     className={`px-4 py-2.5 rounded-lg text-sm font-medium cursor-pointer transition-all ${pickupTime === slot ? 'bg-primary text-deep-espresso ring-2 ring-primary font-bold' : 'bg-surface-container-highest text-on-surface hover:bg-surface-bright'}`}
                   >
                     {slot}
                   </button>
                 ))}
            ...
            <div className="flex items-center justify-between p-5 rounded-xl bg-surface-container-high ring-2 ring-primary">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-on-surface/40">Default Method</div>
                </div>
              </div>
      ...
      <div className="lg:w-[40%]">
        <div className="sticky top-24 bg-surface-container rounded-2xl p-6 md:p-8">
          ...
          >
            {isSubmitting ? <span className="animate-pulse">Processing...</span> : <><LockIcon /> Place Order</>}
          </button>

          <div className="flex items-center justify-center gap-2 mt-5">
            <ShieldIcon />
            <p className="text-[11px] text-on-surface/30 uppercase tracking-widest text-center font-label">
              Mock checkout. No real card charged.
            </p>
          </div>
        </div>
      </div>
