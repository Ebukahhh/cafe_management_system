...
function PersonIcon({ size = 20 }: { size?: number }) {
  return <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>;
}
function ShieldIcon() {
...
function ChevronRightIcon() {
  return <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6" /></svg>;
}
function XIcon() {
  return <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>;
}
function MenuIcon() {
  return <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="18" x2="21" y2="18" /></svg>;
}
...
/* ── Inner panel — shared between desktop sidebar and mobile drawer ── */
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-6 md:p-8 space-y-6">
        <div className="flex items-center justify-between">
          <Link href="/" onClick={onClose} className="text-2xl font-headline text-primary-container">
            Jennifer&apos;s Café
          </Link>
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="md:hidden p-2 rounded-xl text-on-surface/40 hover:text-on-surface hover:bg-surface-container-low transition-colors cursor-pointer"
        ...
        <div className="space-y-4">
          <div className="relative w-20 h-20 group">
            ...
            <Link
              href="/profile"
              onClick={onClose}
              className="absolute bottom-0 right-0 bg-primary-container text-deep-espresso p-1 rounded-full cursor-pointer hover:scale-110 transition-transform"
            >
        ...
        <div className="bg-surface-container-low p-4 rounded-2xl space-y-3" style={{ border: "1px solid rgba(200,134,74,0.1)" }}>
          ...
          <button className="text-[11px] font-bold text-primary uppercase tracking-wider hover:opacity-80 transition-opacity cursor-pointer">
            View Loyalty Card
          </button>
        </div>
      </div>
      ...
      {/* Nav Links */}
      <nav className="flex-1 px-4 py-2 space-y-1 overflow-y-auto" aria-label="Profile navigation">
        {sidebarItems.map((item) => {
          ...
          return (
            <Link
              ...
              style={active ? { borderLeft: "3px solid var(--color-primary, #C8864A)" } : undefined}
            >
              <span className="flex items-center gap-4">
                {item.icon}
                <span className="text-sm">{item.label}</span>
              </span>
              <span className={`transition-opacity ${active ? "opacity-60" : "opacity-0 group-hover:opacity-30"}`}>
                <ChevronRightIcon />
              </span>
            </Link>
          );
      ...
      <div className="p-4" style={{ borderTop: "1px solid rgba(82,68,57,0.05)" }}>
        <a
          href="#"
          className="flex items-center gap-4 px-4 py-3 rounded-xl text-on-surface/30 hover:text-on-surface/60 hover:bg-surface-container-low transition-colors duration-200"
        >
          <GearIcon />
        ...
        <button
          ...
          className="w-full flex items-center gap-4 px-4 py-3 rounded-xl text-on-surface/30 hover:text-red-400 hover:bg-surface-container-low transition-colors duration-200 cursor-pointer"
        >
          <LogoutIcon />
...
/* ── Mobile trigger button — import this in profile pages' mobile header ── */
export function ProfileMenuButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="p-2 rounded-xl text-on-surface/60 hover:text-primary hover:bg-surface-container-low transition-colors cursor-pointer"
...
/* ── Main export: renders fixed sidebar on desktop, drawer on mobile ── */
export default function ProfileSidebar() {
  ...
  // Listen for custom open event from ProfileMobileHeader
  useEffect(() => {
  ...
  return (
    <>
      {/* ── Mobile: backdrop ── */}
      <div
        className={`fixed inset-0 z-[70] md:hidden bg-black/60 backdrop-blur-sm transition-opacity duration-300 ${
      ...
      <div
        className={`fixed top-0 left-0 h-full z-[80] md:hidden w-[min(320px,90vw)] bg-deep-espresso overflow-y-auto transition-transform duration-300 ease-out shadow-2xl ${
          drawerOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
        style={{ borderRight: "1px solid rgba(82,68,57,0.2)" }}
      ...
      <aside
        className="hidden md:flex fixed left-0 top-0 h-full z-40 w-72 bg-deep-espresso flex-col"
        style={{ borderRight: "1px solid rgba(82,68,57,0.1)" }}
      >
        <SidebarPanel />
      </aside>
    </>
